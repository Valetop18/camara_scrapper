...
const guardarDataDiputado = async (diputado, actual, total) => {
  ...
  console.log(
    `[${progreso}] Asistencia guardada: ${asistencia.sesiones.length} registros`,
  );
  console.log(
    `[${progreso}] Asistencias justificadas: ${asistencia.resumen.ausencias_justificadas}`,
  );
  ...
  console.log(
    `[${progreso}] comisiones guardada: ${comisiones.length} registros`,
  );
...
const run = async () => {
  ...
  console.log(
    `Diputados procesados exitosamente ${diputados.length - errores.length} / ${diputados.length}`,
  );
  ...
  if (errores.length > 0) {
    ...
    errores.forEach((error, index) => {
      console.log(`${index + 1}. ID: ${error.id_diputado} | ${error.nombre}`);
      console.log(`   ${error.mensajeError}`);
    });

    console.log("\nIDs de diputados con error:");
    console.log(errores.map((error) => error.id_diputado).join(", "));
  }
